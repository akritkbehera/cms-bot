...
TEST_REGEXP = r"^test(?: workflow(?:s|) (?P<wf>{workflow}(,{workflow}|)*)|)(?: with (?P<pr>{cms_pr}(,{cms_pr})*)|)( for {release_queue}|)(?P<using> using full cmssw| using (?:cms-|)addpkg (?P<pkg>{pkg}(?:,{pkg})*)|)$".format(
    workflow=WF_PATTERN,
...
def setup_logging(loglevel):
    ...
    if not len(logger.handlers):
        ...
        handler2 = logging.FileHandler("process_pr.log")
...
def preprocess_comment_text(comment_msg):
    ...
    comment_lines = [re.sub(r"[ ]{2,}", " ", x) for x in comment_lines]
    comment_lines = [
